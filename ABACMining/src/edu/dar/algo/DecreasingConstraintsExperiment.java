...
public class DecreasingConstraintsExperiment {
	public static int maxNumCon = 3;
	...
	public static void main(String[] args) {
		System.out.println("Decreasing Constraints");
		String outputFile = "ran-case-studies/decreasing_constraints/output_withoutopt.csv";
		try {
			...
			writer.write("NRules, Ncons,|U|mean,|U|stdv,|R|mean,|R|stdv,|O|mean,|O|stdv,"
			...
			for (int numRules = MIN_NRULE; numRules <= MAX_NRULE; numRules += STEP_LEN) {
				...
				for (int k = 0; k < NUMCASES; k++) {
					...
					String inputFile = "ran-case-studies/constraint_"
							+ numRules + "_" + k + ".abac";
					Time.setStartTime();
						...
						while (true) {
							if (SyntheticPolicyCaseStudyGenerator
									.generateCaseStudy(inputFile, null,
											numRules, false, true, 2,
											maxNumCon - 1, 0, 0, 0) != null) {
								break;
							} else {
					...
					// " "
					// + Parser.config.getUsers().size() + " " +
					// Parser.config.numDistintPerms());
					...
					for (int i = maxNumCon - 2; i >= 0; i--) {
						...
						for (Rule rule : config.getRuleList()) {
							if (rule.getCon().size() > 0) {
								int conIndex = rand.nextInt(rule.getCon()
										.size());
						...
						// true, 0, 0.0, false);
						Time.setElapsedTime();
						avgRunningTime[i][k] = (double) Time
								.getElapsedCPUTime();
